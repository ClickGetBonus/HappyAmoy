...
@interface WYDrawCircleProgressView () <CAAnimationDelegate>
...
@implementation WYDrawCircleProgressView
...
- (CAShapeLayer *)trackLayer {
    if (!_trackLayer) {
        ...
        _trackLayer.strokeStart = 0.f; _trackLayer.strokeEnd = 1.f; _trackLayer.path = self.bezierPath.CGPath;
    }
    return _trackLayer;
}
...
- (CAShapeLayer *)progressLayer {
    if (!_progressLayer) {
        ...
        _progressLayer.lineWidth = self.lineWidth ? self.lineWidth : 2.f; _progressLayer.lineCap = kCALineCapRound;
        // 进度条圈进度色
        ...
        pathAnimation.duration = self.animationDuration; pathAnimation.fromValue = @(0.0); pathAnimation.toValue = @(1.0); pathAnimation.removedOnCompletion = YES;
