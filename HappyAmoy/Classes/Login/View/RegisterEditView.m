...
/**    邀请码图标    */
@property (strong, nonatomic) UIButton *invitatedCodeButton;
/**    账号输入框    */
...
/**    邀请码输入框    */
@property (strong, nonatomic) UITextField *invitatedCodeTextField;
...
@implementation RegisterEditView
...
- (void)setupUI {
    ...
    [self.pswButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(line2.mas_bottom).offset(AUTOSIZESCALEX(15));
        make.left.equalTo(self.accountIconButton).offset(0);
        make.width.equalTo(self.accountIconButton);
        make.height.equalTo(self.accountIconButton);
    }];
    ...
    [self settingTextField:pswTextField placeholder:@"请输入密码" placeholderColor:ColorWithHexString(@"#999999")];
    ...
    UIButton *invitatedCodeButton = [[UIButton alloc] init];
    [invitatedCodeButton setImage:ImageWithNamed(@"邀请") forState:UIControlStateNormal];
    [self addSubview:invitatedCodeButton];
    self.invitatedCodeButton = invitatedCodeButton;
    
    [self.invitatedCodeButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(line3.mas_bottom).offset(AUTOSIZESCALEX(15));
        make.left.equalTo(self.accountIconButton).offset(0);
        make.width.equalTo(self.accountIconButton);
        make.height.equalTo(self.accountIconButton);
    }];
    
    UITextField *invitatedCodeTextField = [[UITextField alloc] init];
    [self settingTextField:invitatedCodeTextField placeholder:@"请输入邀请码" placeholderColor:ColorWithHexString(@"#999999")];
    [self addSubview:invitatedCodeTextField];
    self.invitatedCodeTextField = invitatedCodeTextField;
    
    [self.invitatedCodeTextField mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.invitatedCodeButton).offset(AUTOSIZESCALEX(0));
        make.left.equalTo(self.accountTextField).offset(AUTOSIZESCALEX(0));
        make.width.equalTo(self.accountTextField);
        make.height.equalTo(self.accountTextField).offset(0);
    }];
...
- (NSString *)password {
    ...
    _password = self.pswTextField.text;
    
    return _password;
}

- (NSString *)invitatedCode {
    
    _invitatedCode = self.invitatedCodeTextField.text;
    
    return _invitatedCode;
}
