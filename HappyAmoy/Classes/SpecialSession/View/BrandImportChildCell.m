...
@interface BrandImportChildCell()
...
@implementation BrandImportChildCell

- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        [self setupUI];
        
        self.contentView.layer.borderColor = ColorWithHexString(@"#B4ACAC").CGColor;
...
- (void)setupUI {
    ...
    iconImageView.image = [UIImage scaleAcceptFitWithImage:ImageWithNamed(@"省惠拼团banner") imageViewSize:CGSizeMake(AUTOSIZESCALEX(130), AUTOSIZESCALEX(130))];
...
- (void)addConstraints {
    ...
    [self.ticketButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.iconImageView.mas_bottom).offset(AUTOSIZESCALEX(12));
    ...
    [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.ticketButton.mas_bottom).offset(AUTOSIZESCALEX(10));
        make.left.equalTo(self.iconImageView).offset(AUTOSIZESCALEX(0));
