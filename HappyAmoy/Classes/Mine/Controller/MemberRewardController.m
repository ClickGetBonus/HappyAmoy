...
@interface MemberRewardController () <UITableViewDelegate,UITableViewDataSource,RewardBalanceViewDelegate,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>

/**    tableView    */
@property (strong, nonatomic) UITableView *tableView;
/**    我的余额    */
@property (strong, nonatomic) RewardBalanceView *balanceView;
/**    余额明细数据源    */
...
@implementation MemberRewardController
...
- (void)setupBalanceView {
    ...
    RewardBalanceView *balanceView = [[RewardBalanceView alloc] init];
...
- (void)getBalance {
    ...
    [[NetworkSingleton sharedManager] getRequestWithUrl:@"/personal/reward" parameters:parameters successBlock:^(id response) {
        if ([[response objectForKey:@"code"] integerValue] == RequestSuccess) {
            weakSelf.balanceView.balanceText = [NSString stringWithFormat:@"%@",[response objectForKey:@"data"]];
        }
...
- (void)getRecordWithPageNo:(NSInteger)pageNo {
    ...
    [[NetworkSingleton sharedManager] getRequestWithUrl:@"/personal/rewards" parameters:parameters successBlock:^(id response) {
...
- (void)rewardBalanceView:(RewardBalanceView *)rewardBalanceView toWallet:(NSInteger)currentScore {
    ...
    if ([LoginUserDefault userDefault].userItem.viped == 0) {
                [WYProgress showErrorWithStatus:@"升级VIP才能使用"];
        
        UpgradeVIPController *webVc = [[UpgradeVIPController alloc] init];
        [weakSelf.navigationController pushViewController:webVc animated:YES];
        
                return;
    }
    NSString *qrurl = [[NSString alloc] initWithFormat:@"%@%@%@",@"http://haomaih5.lucius.cn/#/Convert?phone=", [LoginUserDefault userDefault].userItem.mobile,@"&tab=1&type=0"];
