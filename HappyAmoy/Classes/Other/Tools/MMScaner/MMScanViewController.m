...
//        _tipTitle.center = CGPointMake(self.view.center.x, self.view.center.y + CGSizeFromString([self scanRectWithScale:1][1]).height/2 + 25);
        ...
        _tipTitle.center = CGPointMake(self.view.center.x, self.view.center.y * 1.35);
...
- (void)renderUrlStr:(NSString *)url {
    ...
    if (self.scanFinish) {
        ...
        if (!self.isCheckTicket) {
            CKScanResultController *scanResultVc = [[CKScanResultController alloc] init];
            scanResultVc.scanResult = url;
...
#warning 修改位置
    [_flashBtn setCenter:CGPointMake(self.view.center.x, self.view.center.y * 1.5)];
