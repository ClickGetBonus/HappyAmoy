...
@interface MyOrderOfGroupBuyController () <UITableViewDelegate,UITableViewDataSource>
...
@implementation MyOrderOfGroupBuyController
...
- (void)getDataWithPageNo:(NSInteger)pageNo {
    ...
    WeakSelf
    [[NetworkSingleton sharedManager] getRequestWithUrl:@"/group/groupOrders" parameters:parameters successBlock:^(id response) {
        if ([response[@"code"] integerValue] == RequestSuccess) {
            if (pageNo == 1) {
                weakSelf.pageNo = 1;
                weakSelf.datasource = [GroupOrderItem mj_objectArrayWithKeyValuesArray:response[@"data"][@"datas"]];
                [weakSelf.tableView reloadData];
            } else {
                [weakSelf.datasource addObjectsFromArray:[GroupOrderItem mj_objectArrayWithKeyValuesArray:response[@"data"][@"datas"]]];
            }
...
- (void)setupUI {
    ...
    [tableView registerClass:[MyOrderOfGroupBuyCell class] forCellReuseIdentifier:orderCellId];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MyOrderOfGroupBuyCell *cell = [tableView dequeueReusableCellWithIdentifier:orderCellId];
...
//    GroupOrderItem *item = self.datasource[indexPath.section];
//    detailVc.groupId = item.orderId;
