...
@interface GoodsListOfNewController () <UIScrollViewDelegate>
...
@implementation GoodsListOfNewController
...
//    self.categoriesArray = @[@"精选",@"女装",@"美妆",@"食品",@"护肤",@"数码",@"内衣",@"女鞋",@"母婴",@"家电",@"运动",@"宠物"];
...
- (void)setupChildVc {
    for (int i = 0; i < self.categoriesArray.count; i++) {
        SpecialCategoryFeaturedController *childVc = [[SpecialCategoryFeaturedController alloc] init];
        childVc.isVideoType = self.isVideoType;
...
//            [self addChildViewController:childVc];
//        } else {
//            GoodsListChildController *childVc = [[GoodsListChildController alloc] init];
//            childVc.specialCategoryId = self.specialCategoriesId;
//            childVc.classifyItem = self.categoriesArray[i];
//            //        childVc.isFeatured = i == 0;
//            [self addChildViewController:childVc];
//        }
    }
}
