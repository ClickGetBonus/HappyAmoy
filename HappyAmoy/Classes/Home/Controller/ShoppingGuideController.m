...
#import "LQZNController.h"
#import "RHSQController.h"
#import "XSBKController.h"

@interface ShoppingGuideController () <UIScrollViewDelegate>
...
@implementation ShoppingGuideController
...
- (void)viewDidLoad {
    ...
    bgImageView.image = ImageWithNamed(@"图层1");
...
#pragma mark - 添加子控制器
- (void)setupChildVc {
    TBCQController *tbcqVc = [[TBCQController alloc] init];
    [self addChildViewController:tbcqVc];
    
    LQZNController *lqznVc = [[LQZNController alloc] init];
    [self addChildViewController:lqznVc];

    RHSQController *rhsqVc = [[RHSQController alloc] init];
    [self addChildViewController:rhsqVc];

    XSBKController *xsbkVc = [[XSBKController alloc] init];
    [self addChildViewController:xsbkVc];
}
...
- (void)setupTitleView {
    ...
    UIView *titleView = [[UIView alloc] initWithFrame:CGRectMake(0, kNavHeight, SCREEN_WIDTH, AUTOSIZESCALEX(45))];
...
- (void)setupTitleButton {
    ...
    NSArray *titleArray = @[@"淘宝查券",@"领券指南",@"如何省钱",@"新手必看"];
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        [button setTitleColor:QHBlackColor forState:UIControlStateSelected];
        
        [button setTitle:titleArray[i] forState:UIControlStateNormal];
        button.titleLabel.font = TextFont(15);
